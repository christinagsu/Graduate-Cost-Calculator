...
function calcCost(formData, jqForm, options) {	
           ...
           var tuition_val = "[Please select enrollment, tuition classification, program, and specialized program (if applicable) in the section above]";
           var residency=false;
           var hope_books = 0; 
           var hope_fee = 0;
           var hope_coverd = 0;
           ...
           if (residency) {				   
					   ...
					   if(form.entrystatus.value == "transfer_oos") {          
						 ...
						 if(form.residency.value == "oos") tuition_cost= 12873 * form.enrollment.value;             
                                           ...
                                           var residencybox=document.getElementById("residency");
                                           var base_cost=0;
                                           var programbox=document.getElementById("program");
                                           if (residencybox.options[residencybox.selectedIndex].value=="oos"){
                                             if (programbox.options[programbox.selectedIndex].value=="graduate"){
                                                      base_cost=10800;
                                             }
                                                      else{
                                                               var specialprogrambox=document.getElementById("specialized_program");
                                                               var specialized_program=specialprogrambox.options[specialprogrambox.selectedIndex].value;
                                                               if (specialized_program=="mba") base_cost=11241;
                                                               else if (specialized_program=="ms") base_cost=10854;
                                                               else if (specialized_program=="clinical") base_cost=10881;
                                                               else if (specialized_program=="master_pubhealth") base_cost=11043;
                                                               else if (specialized_program=="doctor_pubhealth") base_cost=11043;
                                                               else if (specialized_program=="law") base_cost=12222;
                                                               else if (specialized_program=="other") base_cost=document.getElementById("other_specialized_program").value;
                                                      }
                                           }
                                           else{
                                           if (programbox.options[programbox.selectedIndex].value=="graduate")
                                             base_cost=3024;
                                                      ...
                                                      var specialprogrambox=document.getElementById("specialized_program");
                                                      var specialized_program=specialprogrambox.options[specialprogrambox.selectedIndex].value;
                                                      if (specialized_program=="mba") base_cost=3933;
                                                      else if (specialized_program=="ms") base_cost=3078;
                                                      else if (specialized_program=="clinical") base_cost=3105;
                                                      else if (specialized_program=="master_pubhealth") base_cost=3267;
                                                      else if (specialized_program=="doctor_pubhealth") base_cost=3267;
                                                      else if (specialized_program=="law") base_cost=4842;
                                                      else if (specialized_program=="other") base_cost=document.getElementById("other_specialized_program").value;
                                             }
                                           }
                                           tuition_cost=base_cost * form.enrollment.value;
           }
       
           var book_cost=0;
           if ((specialized_program) && specialized_program=="law") book_cost=form.enrollment.value * 625;
           else book_cost= form.enrollment.value * 500.00;
           
           var room_and_board=0;
           if ((specialized_program) && specialized_program=="law") housing_cost=form.enrollment.value * 4885.50;
           else housing_cost= form.enrollment.value * 4612.50;
           document.getElementById("room_and_board").innerHTML=currency(housing_cost);
       
           //var studentfee_cost= form.enrollment.value * 1064.00;
           var studentfee_cost=0;
           if ((specialized_program) && specialized_program=="law") studentfee_cost=form.enrollment.value * 1121;
           else studentfee_cost= form.enrollment.value * 1064;
           ...
           if(isNumeric(form.scholarships.value)  && form.scholarships.value.length >0) 
               ...
               if(isNumeric(form.gradassistant.value)  && form.gradassistant.value.length >0) 
               aid_total = (parseInt(form.gradassistant.value) +  aid_total) *1;
               
               if(isNumeric(form.outofstate_waiver.value)  && form.outofstate_waiver.value.length >0) 
               aid_total = (parseInt(form.outofstate_waiver.value) +  aid_total) *1; 
...
$(document).ready(function(){ 
      ...
      $('.boxyinfo').click(function() {
            ...
            var info_url="wp-content/plugins/grad-calculator/graduate-info.html #"+this.id;
...
function updateVisaType(drpdwn){
         if (drpdwn.selectedIndex==2)
                  document.getElementById('agecon').style.display='';
         else{
                  document.getElementById('age').selectedIndex=0;
                  document.getElementById("insurance_cost").innerHTML=formatCurrency(0);
                  document.getElementById("tempinsurancecost").value=0;
...
function changeResidency(drpdown){
         if (drpdown.selectedIndex==2) {
                  document.getElementById('visatypecon').style.display='';
                  document.getElementById('out_of_state_waiver_con').style.display='';
         }
         else{
                  document.getElementById('visatypecon').style.display='none';
                  document.getElementById('out_of_state_waiver_con').style.display='none';
                  document.getElementById('outofstate_waiver').value=0;
                  document.getElementById('age').selectedIndex=0;
                  document.getElementById("insurance_cost").innerHTML=formatCurrency(0);
                  document.getElementById("tempinsurancecost").value=0;
                  document.getElementById('agecon').style.display='none';
